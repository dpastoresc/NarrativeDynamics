...
def quitar_tildes(text):
    ...
    vocales_tildes = [('á','a'), ('é','e'), ('í','i'), ('ó','o'), ('ú','u'), ('ü','u')]
    for s,t in vocales_tildes:    
...
def clean_text(text):
    ...
    text_til = quitar_tildes(text_pp)
    ...
    #Carlota: Igual esto es más optimo si se añade la lista de nombres en vez de hacer dos veces el lambda no?
    stopwords_spanish.extend(spanish_names)
...
dfProcessed = dfwhole.loc[:,('tweet_id','tokens_text','hashtags')]


#Save the processed dataframe to pickle - folder name dfs
path_dfs = ''
dfProcessed.to_pickle(datapath+path_dfs+ db_name_table+'Processed_s.pkl')

allwords=df['tokens_text'].to_list()
allwords.extend(df['hashtags'].to_list())

#List of lists including all words --> frequency distribution
corpus_Post = list(itertools.chain.from_iterable(allwords))
...
path_dicts = ''
with open(datapath+path_dicts+'distFreq'+db_name_table+'_s.pkl', 'wb') as f:
